...
const CAUSE_LIBRARY: Record<
  CauseId,
  {
    title: string;
    short: string;
    pkgBias?: Partial<Record<PackageType, number>>;
    tests: TestCard[];
  }
> = {
  RINSE_DRY_BOUNDARY: {
    ...
    tests: [
      {
        ...
        next: "Lock optimized rinse/dry; verify hardware alignment; gate-release after clean monitors",
      },
      {
        test: "Edge-inclusive AOI review + drying map",
        signal: "Strong edge-band/ring signature; non-uniform dry map",
        means: "Drying/coverage issue",
        next: "Check dryer/N2 knife uniformity; verify drain/spin sequence",
      },
    ],
  },
  ...
  WETTING_ORGANIC_CONTAM: {
    ...
    tests: [
      {
        test: "Quick wetting check: water-break / contact angle on affected vs baseline",
        signal: "Poor wetting / patchy film formation",
        means: "Organic contamination/wetting failure",
        next: "Clean/refresh surfaces; tighten handling; add pre-wet/clean step; gate-release",
      },
      {
        test: "SEM/EDS + solvent/clean trial",
        signal: "Organic signature; residue reduces after solvent/clean",
        means: "Organic-driven film",
        next: "Audit materials/handling; update clean recipe; hold until monitors pass",
      },
    ],
  },
  ...
  PARTICLE_SHEDDING: {
    ...
    tests: [
      {
        test: "Hardware check: seals/nozzle/chuck/lines + inspect filters/DP trend",
        signal: "DP spike / debris found / recent part swap correlates",
        means: "Particle source confirmed",
        next: "Replace/clean source; run monitors; quarantine window until pass",
      },
      {
        test: "SEM morphology + EDS across multiple islands",
        signal: "Discrete particles with consistent composition",
        means: "Shedding/contamination event",
        next: "Trace to module; add purge/flush; tighten PM re-assembly checks",
      },
    ],
  },
  ...
  REDEPOSITION_TRAPPED_CHEM: {
    ...
    tests: [
      {
        test: "Dense vs sparse comparison: AOI ratio + SEM cross-check",
        signal: "Dense areas significantly worse",
        means: "Transport/trap mechanism",
        next: "Boost rinse/megasonic (if available) / dwell; add post-step clean split",
      },
      {
        test: "Split: standard vs enhanced post-step clean/DI cascade",
        signal: "Residue drops with enhanced clean",
        means: "Trapped chem / redeposition",
    ...
    tests: [
      {
        ...
        next: "Block PR lot; tune strip/descum; re-qualify materials before release",
      },
      {
        test: "EDS + O2 plasma/descum trial on monitor",
        signal: "Organic dominant; residue removed by descum",
        means: "Resist scum",
        next: "Update strip/descum controls; add incoming PR checks",
      },
    ],
  },
  ...
  BP_TRANSFER_NOZZLE: {
    ...
    tests: [
      {
        test: "BP transfer monitor: run dummy/monitor through placement module then inspect",
        signal: "Transfer signature repeats on monitors",
        means: "Placement head/nozzle contamination",
        next: "Clean/replace nozzle/head parts; verify with 2 clean monitors; gate-release",
      },
      {
        test: "Inspect head/nozzle + cleaning logs; check recent part changes",
        signal: "Condition/cleaning gap correlates",
        means: "Module-side contamination",
        next: "Tighten PM/clean frequency; add pre-run purge/clean step",
      },
    ],
  },
  ...
  BP_HANDLER_CONTACT: {
    ...
    tests: [
      {
        test: "Map defect vs contact points; inspect end-effectors / trays / gloves",
      ...
      {
        test: "Swap consumables (tray/end-effector) and re-run monitor",
        signal: "Immediate improvement after swap",
        means: "Consumable source",
        next: "Update consumable life limits; add incoming inspection",
      },
    ],
  },
  ...
  EP_BATH_AGING_ADDITIVES: {
    ...
    tests: [
      {
        test: "Pull bath health + trend vs drift start",
        signal: "Out-of-control parameter correlates with defect drift",
        means: "Bath aging/additive breakdown",
        next: "Refresh bath/additives; verify via monitors; gate-release after pass",
      },
      {
        test: "Filter DP + circulation check",
        signal: "DP increases as drift worsens",
        means: "Bath cleanliness degrading",
        next: "Change filters; flush loop; monitor defect trend",
      },
    ],
  },
    ...
    tests: [
      {
        ...
        signal: "Debris found; DP spike aligns to window",
        means: "EP particle source confirmed",
        next: "Replace bags/filters; flush; run 2 clean monitors; gate-release",
      },
      {
        test: "SEM/EDS particles: compare to bath/anode materials",
        signal: "Composition matches anode/bag/filter media",
        means: "Source attribution strong",
        next: "Lock CA; add DP alarm limits",
      },
    ],
  },
  ...
  EP_MICROLOADING_TRANSPORT: {
    ...
    tests: [
      {
        test: "Dense vs sparse split + adjust agitation/flow (within spec) on monitor",
        signal: "Dense-bias reduces with improved transport",
        means: "Microloading/transport limitation",
        next: "Optimize agitation/flow; enhance post-plate rinse; gate-release after monitors",
      },
      {
        test: "Enhanced rinse/dry split after EP step",
        signal: "Defects drop with boosted rinse/dry",
        means: "Transport/trap driven",
        next: "Standardize boosted rinse/dry; monitor long-term",
      },
    ],
  },
};
...
function scoreCauses(input: CaseInput) {
  ...
  if (input.processStep === "POST_STRIP_DESCUM") {
    add("PR_STRIP_SCUM", 30, "Process step=post-strip/descum");
    add("WETTING_ORGANIC_CONTAM", 10, "Strip/descum sensitive to organics");
    base.EP_BATH_AGING_ADDITIVES *= 0.7;
    base.EP_MICROLOADING_TRANSPORT *= 0.75;
    triggers.push("EP bath/microloading: down-weighted (found post-strip/descum)");
  }

  if (input.processStep === "POST_EP_RINSE_DRY") {
    add("RINSE_DRY_BOUNDARY", 25, "Process step=post-EP rinse/dry");
    add("EP_BATH_AGING_ADDITIVES", 14, "Post-EP sensitive to bath health");
    add("EP_MICROLOADING_TRANSPORT", 10, "Post-EP density/transport effects");
  }

  if (input.processStep === "POST_ETCH_CLEAN") {
    add("REDEPOSITION_TRAPPED_CHEM", 16, "Process step=post-etch clean");
    add("RINSE_DRY_BOUNDARY", 10, "Post-clean boundary effects");
    add("WETTING_ORGANIC_CONTAM", 8, "Post-clean wetting failure");
  }
  ...
  if (input.processStep === "FINAL_INSPECTION") {
    add("RINSE_DRY_BOUNDARY", 10, "Final inspection reflects rinse/dry history");
    add("WETTING_ORGANIC_CONTAM", 10, "Final inspection organics/handling accumulate");
    add("PARTICLE_SHEDDING", 8, "Final inspection particle addition possible");
  }
  ...
  if (input.morphology === "RING") add("RINSE_DRY_BOUNDARY", 24, "Morphology=ring");
  if (input.morphology === "HAZE_FILM") {
    add("RINSE_DRY_BOUNDARY", 18, "Morphology=haze/film");
    add("WETTING_ORGANIC_CONTAM", 16, "Film can be wetting/organic related");
  }
  ...
  if (input.morphology === "STREAKS") {
    add("RINSE_DRY_BOUNDARY", 12, "Morphology=streaks suggests flow/dry");
    add("WETTING_ORGANIC_CONTAM", 10, "Streaking can indicate wetting failure");
  }
  ...
  if (input.toolCorr === "MULTI") {
    add("EP_BATH_AGING_ADDITIVES", 14, "Multi-tool suggests systemic chemistry/loop");
    add("WETTING_ORGANIC_CONTAM", 8, "Systemic handling/materials possible");
  }

  // Time behavior
  if (input.timeBehavior === "STEP_CHANGE") {
    if (input.changes.POST_PM_OR_PARTS) {
      add("PARTICLE_SHEDDING", 18, "Step-change after PM/parts");
      add("RINSE_DRY_BOUNDARY", 12, "PM can affect shields/centering/coverage");
      add("WETTING_ORGANIC_CONTAM", 10, "PM can introduce organics/wetting shift");
    } else {
      add("PARTICLE_SHEDDING", 10, "Step-change suggests event-driven source");
    }
  }
  if (input.timeBehavior === "INTERMITTENT") {
    add("PARTICLE_SHEDDING", 16, "Intermittent matches shedding/transfer");
    add("BP_HANDLER_CONTACT", 10, "Intermittent handling/contact");
    add("EP_ANODE_FILTER_PARTICLES", 10, "Intermittent EP particles");
  }
  if (input.timeBehavior === "DRIFT") {
    add("EP_BATH_AGING_ADDITIVES", 22, "Drift suggests bath aging/additives");
    add("WETTING_ORGANIC_CONTAM", 8, "Gradual contamination buildup possible");
  }
  ...
  if (input.changes.RECIPE_CHANGE) {
    add("RINSE_DRY_BOUNDARY", 10, "Recipe change affects rinse/dry boundary");
    add("REDEPOSITION_TRAPPED_CHEM", 8, "Recipe change alters transport/clean");
  }
  ...
  if (input.changes.PRODUCT_MIX_CHANGE) {
    add("REDEPOSITION_TRAPPED_CHEM", 10, "Mix changes stress rinse/clean");
    add("EP_MICROLOADING_TRANSPORT", 10, "Different patterns change transport");
  }

  // Chemistry age
  if (input.pkg === "EP") {
    if (input.chemAge === "LATE") add("EP_BATH_AGING_ADDITIVES", 20, "Chem age=late (EP)");
    if (input.chemAge === "MID") add("EP_BATH_AGING_ADDITIVES", 8, "Chem age=mid (EP)");
  } else {
    if (input.chemAge === "LATE") add("WETTING_ORGANIC_CONTAM", 8, "Chem age=late (upstream clean)");
  }
  ...
  const stopNow =
    ...
    (input.timeBehavior === "STEP_CHANGE" &&
      input.toolCorr === "SINGLE" &&
      input.changes.POST_PM_OR_PARTS &&
      (input.location === "EDGE_BAND" || input.morphology === "RING"));
...
function buildFirstMoves(input: CaseInput, top3: Array<{ id: CauseId; pct: number }>) {
  ...
  if (input.bridgingRisk || input.evidence === "METAL_CONFIRMED") {
    ...
    moves.push("HOLD suspect time window; prevent escapes with edge-inclusive AOI/extra sampling until gate passes.");
  }

  moves.push("Run AOI with edge-inclusive recipe + capture signature (edge vs center, dense vs sparse).");

  if (input.evidence === "NOT_DONE") {
    moves.push("Do SEM + EDS on worst sites: (1) dense, (2) sparse, (3) edge band if present; classify metal vs organic vs salt/oxide.");
  } else {
    moves.push("Confirm mechanism: replicate 1–2 wafers SEM/EDS across regions to ensure not a one-off artifact.");
  }

  const top = top3[0]?.id;
  if (top === "RINSE_DRY_BOUNDARY") {
    moves.push("Run monitor split: normal vs +2× DI rinse + dry tweak (spin/N2 knife/time); compare edge/ring response.");
  } else if (top === "PARTICLE_SHEDDING" || top === "EP_ANODE_FILTER_PARTICLES") {
    moves.push("Check filters/DP + inspect suspect parts (nozzles/seals/lines/anode bag); run 2 clean monitors after cleanup.");
  } else if (top === "PR_STRIP_SCUM") {
    moves.push("Material/strip split: old PR vs new PR AND standard vs boosted strip/descum; re-inspect before restart.");
  } else if (top === "EP_BATH_AGING_ADDITIVES") {
    moves.push("Pull EP bath health + filter DP; correlate to drift start; refresh as needed; verify with monitors.");
  } else if (top === "EP_MICROLOADING_TRANSPORT" || top === "REDEPOSITION_TRAPPED_CHEM") {
    moves.push("Split: standard vs enhanced post-step clean/rinse (and/or agitation/flow within spec); compare dense vs sparse response.");
  } else if (top === "BP_TRANSFER_NOZZLE" || top === "BP_HANDLER_CONTACT") {
    moves.push("BP check: run dummy/monitor through placement/handler; inspect transfer signature; clean/replace contact surfaces.");
  } else {
    moves.push("Run monitor split: normal vs extended rinse/dry; use response to isolate boundary vs contamination mechanisms.");
  }
...
export default function Home() {
  ...
  // ✅ AUTO-REFRESH: anytime gate OR any inputs change AFTER Analyze has been run
  useEffect(() => {
    if (!output) return;
    recompute();
  ...
  return (
    <div className="min-h-screen relative overflow-hidden bg-[#05070d] text-white">
      ...
      <div className="pointer-events-none absolute inset-0">
        <div className="absolute -top-40 left-1/2 h-[700px] w-[700px] -translate-x-1/2 rounded-full bg-green-500/10 blur-3xl animate-pulse" />
        <div className="absolute -bottom-56 right-[-120px] h-[700px] w-[700px] rounded-full bg-cyan-500/10 blur-3xl animate-pulse" />
        <div className="absolute inset-0 opacity-[0.08] [background:linear-gradient(to_right,rgba(255,255,255,0.35)_1px,transparent_1px),linear-gradient(to_bottom,rgba(255,255,255,0.35)_1px,transparent_1px)] [background-size:48px_48px]" />
      ...
      <div className="relative mx-auto flex min-h-screen max-w-6xl flex-col px-6 py-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* Left: Form */}
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl shadow-[0_25px_90px_rgba(0,0,0,0.55)]">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="flex items-end">
                <Toggle value={denseBias} onChange={setDenseBias} label="Dense pattern worse" hint="If dense areas show higher residue/island count" />
              </div>
            </div>
            ...
            <div className="mt-4">
              <Toggle value={bridgingRisk} onChange={setBridgingRisk} label="Bridging risk" hint="If residue could cause shorts/bridge (high risk)" />
            </div>
            ...
            <div className="mt-6 text-xs text-white/35">
              Tip: Click Analyze once, then tick gates as you “execute the war-room plan” → watch verdict shift to RELEASE.
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-xl shadow-[0_25px_90px_rgba(0,0,0,0.55)]">
            ...
            <div className="relative rounded-xl border border-white/10 bg-black/55 p-4 min-h-[720px]">
              {isThinking && (
        ...
        <div className="mt-10 text-center text-xs text-white/30">
          © 2026 Thurkhesan Murugan • Fab Experience — Taiwan 🇹🇼
        </div>
      </div>
    </div>
  );
